...
void UboLight::Update(std::shared_ptr<Light> light) {
  ...
  glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(glm::vec3),
                  glm::value_ptr(light->GetPos())); // pos
  // std::cout << glm::to_string(light[0]->GetColor());
  glBufferSubData(GL_UNIFORM_BUFFER, 16, sizeof(glm::vec3),
                  glm::value_ptr(light->GetDir())); // dir
  glBufferSubData(GL_UNIFORM_BUFFER, 32, sizeof(glm::vec3),
                  glm::value_ptr(light->GetColor()));         // color
