...
#include <glm/glm.hpp>
#include <vector>
SSAO::SSAO(uint32_t w, uint32_t h) : width(w), height(h) {
...
void SSAO::SetSSAOFactor(float _factor) { 
    factor = _factor; 
}
void SSAO::Draw(std::shared_ptr<Framebuffer> color_fbo, uint32_t pos_tex, uint32_t normal_tex,
