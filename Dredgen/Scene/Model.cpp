...
void Model::Draw(Shader &shader, RenderMode _rendermode = Triangle) {
...
Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene) {
    ...
    for (uint32_t i = 0; i < mesh->mNumVertices; i++) {
        ...
        if (mesh->HasNormals()) {
            ...
            Log::Log(mesh->mName.C_Str(), "no normal");
        }
        // texture coordinates
...
uint32_t TextureFromFile(const char *path, const std::string &directory, [[maybe_unused]] bool gamma) {
